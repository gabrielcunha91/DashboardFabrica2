...
#   df_faturam_zig['Antecipacao_Credito'] = df_faturam_zig['Loja'].apply(lambda x: 0 if x == 'Arcos' else 1) #colocar tbm jacaré, lobe, bar leo e blue note
...
#   df_faturam_zig['Data_Pagamento'] = df_faturam_zig.apply(lambda row: calcula_data_pagamento(row['Data_Faturamento'], row['Tipo_Pagamento'], serie_datas_feriados), axis=1)

#   df_faturam_zig = df_faturam_zig.rename(columns={'Data_Pagamento': 'Data'})

#   return df_faturam_zig

# # Função para calcular a data de pagamento com base no tipo de pagamento
# def calcula_data_pagamento(data_faturamento, tipo_pagamento, serie_datas_feriados):
#   # Definindo os dias de compensação
#   dias_compensacao = {
#     'DÉBITO': 1,
...
#   dias = dias_compensacao.get(tipo_pagamento, 0)
#   data_pagamento = data_faturamento + pd.DateOffset(days=dias)

#   while data_pagamento in serie_datas_feriados or data_pagamento.weekday() >= 5:
#     data_pagamento += pd.DateOffset(days=1)
...
#   # Usando o dicionário de taxas para calcular a taxa com base no tipo de pagamento e antecipação
#   df_faturam_zig['Taxa'] = df_faturam_zig.apply(lambda row: 
#                                           row['Valor_Faturado'] * taxas['DÉBITO'] if row['Tipo_Pagamento'] == 'DÉBITO' else
#                                           row['Valor_Faturado'] * taxas['CRÉDITO_ANTECIPADO'] if row['Tipo_Pagamento'] == 'CRÉDITO' and row['Antecipacao_Credito'] == 1 else
#                                           row['Valor_Faturado'] * taxas['CRÉDITO_PADRAO'] if row['Tipo_Pagamento'] == 'CRÉDITO' and row['Antecipacao_Credito'] == 0 else
#                                           row['Valor_Faturado'] * taxas['APP'] if row['Tipo_Pagamento'] == 'APP' else
#                                           row['Valor_Faturado'] * taxas['PIX'] if row['Tipo_Pagamento'] == 'PIX' else
#                                           0.00, axis=1)
...
#   # Antecipado, Pix, Dinheiro, App, Voucher
#   df_faturam_zig['Data_Compensacao'] = df_faturam_zig.apply(lambda row: row['Data_Compensacao'] + pd.Timedelta(days=1) 
#                                                       if row['Tipo_Pagamento'] in ['PIX', 'DINHEIRO', 'APP', 'VOUCHER', 'ANTECIPADO'] else row['Data_Compensacao'], axis=1)
...
def ajustar_data_compensacao(row, serie_datas_feriados):
  if row['Tipo_Pagamento'] == 'DÉBITO':
    ...
    row['Data_Compensacao'] += pd.Timedelta(days=30)
  elif row['Tipo_Pagamento'] in ['PIX', 'DINHEIRO', 'APP', 'VOUCHER', 'ANTECIPADO']:
    row['Data_Compensacao'] += pd.Timedelta(days=1)
